...
public class ManoeuvreActivity extends BaseActivity implements ViewPager.OnPageChangeListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_manoeuvre);
        initView();
        initData();
    }
    ...
    private void initView() {
        mViewPager = findViewById(R.id.iv_manoeuvre);
    }
    ...
    private void beginRecycle(View view) {
        if (!isRecycling && photos.size() > 0) {
            isRecycling = true;
            mViewPager.setAdapter(new MyAdapter(photos, ManoeuvreActivity.this));
            new Thread() {
                @Override
                public void run() {
                    super.run();
            ...
            ShowToastUtils.ShowToast(ManoeuvreActivity.this,"当前文件夹下没有图片文件！", Toast.LENGTH_SHORT);
            isRecycling = false;
        }
    }
    ...
    private void selectPicture(View view) {
        File[] files = FileUtils.getFileList(FileUtils.DirPathForManoeuvre());
