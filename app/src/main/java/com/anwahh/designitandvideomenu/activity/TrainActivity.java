...
public class TrainActivity extends BaseActivity implements ViewPager.OnPageChangeListener {
    ...
    private void initView() {
        mViewPager = findViewById(R.id.iv_train);
    }
    ...
    private void beginRecycle(View view) {
        if (!isRecycling && photos.size() > 0) {
            isRecycling = true;
            mViewPager.setAdapter(new MyAdapter(photos, TrainActivity.this));
            new Thread() {
                @Override
                public void run() {
                    super.run();
            ...
            ShowToastUtils.ShowToast(TrainActivity.this,"当前文件夹下没有图片文件！", Toast.LENGTH_SHORT);
            isRecycling = false;
        }
    }
    ...
    private void selectPicture(View view) {
        File[] files = FileUtils.getFileList(FileUtils.DirPathForTrain());
